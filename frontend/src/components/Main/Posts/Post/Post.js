...
const Post = ({ postInfo, setSelectedPostId }) => {
  ...
  const userLiked = postInfo.likes.find(
  ...
  const Likes = () => {
    if (postInfo.likes.length > 0) {
      return userLiked === "true" ? (
        <h5 className="likesnb">
          {postInfo.likes.length > 2
            ? `You and ${postInfo.likes.length - 1} others`
            : `${postInfo.likes.length} like${
                postInfo.likes.length > 1 ? "s" : ""
              }`}
        </h5>
      ) : (
        <h5 className="likesnb">
    ...
    <div className="post">
      <div className="post_top_container" onClick={handleClickPost}>
        ...
        <div className="post_top">
          ...
          <div className="edit">
            {(user?.result?.sub === postInfo?.creatorID ||
              user?.result?._id === postInfo?.creatorID) &&
            user?.result ? (
              <EditBtn
