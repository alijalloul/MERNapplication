...
const PostDetails = () => {
    ...
    useEffect(() => {
        if(postInfo !== null){
            fetchPostsBySearch({search: null, tags: postInfo?.tags?.join(",")}, page, dispatch);
        }
    },[postInfo]);
            ...
            <div className='post_details'>
                <div className="top_post_details_container">
                    <div className="left_post_details_container">
                        ...
                        <span className="tags">{postInfo.tags.map(tag => `#${tag} `)}</span>
                        <span className="discreption">{postInfo.message}</span>
                        <span className="creatorName">Created By: {postInfo.name}</span>
                        <span className="createdAt">{moment(postInfo.createdAt).fromNow()}</span>
                        ...
                        <div className="bottom_posts_details">
                            {
                                recommendedPosts.map((post) => (
                                    <div className="bottom_post_details" key={post._id}>
                                        ...
                                        <span className="recommendedPost_name">Created By: {post.name}</span>
                                        <span className="recommendedPost_message">{post.message}</span>
                                        <span className="recommendedPost_likes">Likes: {post.likes.length}</span>
                                        <div className="recommendedPost_image_container">
                                            <img src={(post.file == null ) ? postImage : ( (!post.file) ? postImage : post.file)} alt="N/A" onClick={ () =>  handleImageClick(post._id) }/>
                                        </div>
                                    </div>
                                ))
                            }
